...
for function_names, function_body in ASSEMBLY_FUNCTIONS.items():
    for function_name in function_names:
        BUILDIN_FUNCTIONS[function_name] = [
...
class Compiler:
    def __init__(self, file_name):
        ...
        self.counters = dict.fromkeys(["mark", "error", "indent", "lambda"], 0)
    ...
    def compile(self, node):
        if not isinstance(node, list):
            return getattr(
                self, f"compile_{node.__class__.__name__}", self.no_compile_method
            )(node)

        # .enter и .leave нужны для очистки от мусора на стеке (к примеру, инкремент/декремент)
        for expression in node:
            if isinstance(
                expression, (BinaryOperationNode, UnaryOperationNode, ReturnNode)
            ):
                self.enter()
    ...
    def compare(self, first, second, condition, mark_index=None, mark_name="mark"):
        ...
        self.operations(
            ["cmp", first, second], [f"j{condition}", f"{mark_name}{mark_index}"]
        )
    ...
    def replace_mark(
        self,
        replaceable_mark_index,
        replaceable_mark_name,
        substitute_mark_index=None,
        substitute_mark_name="mark",
    ):
    ...
    def compile_UnaryOperationNode(self, node: UnaryOperationNode):
        if node.operator.check_keyword(NOT):
            ...
            self.compile(
                BinaryOperationNode(
                    node.node, Token(EQUAL), NumberNode(Token(INTEGER, "0"))
                )
            )
        elif node.operator.check_type(SUBTRACTION):
    ...
    def compile_VariableAccessNode(self, node: VariableAccessNode):
        ...
        self.mark()
        self.compare(
            "rax", "!LIST_IDENTIFIER", "ne", self.counters["indent"], "list_mark"
        )
        ...
        if node.keys:
            ...
            while keys:
                ...
                self.operations(["imul", "rdx", 8 * 2], ["add", "rdi", "rdx"])

                self.movs(["rax", "[rdi]"], ["rbx", "[rdi - 8]"])
    ...
    def compile_VariableAssignNode(self, node: VariableAssignNode):
        ...
        self.movs(
            ["rcx", f"[{variable}]"], ["rdx", int(variable not in self.variables)]
        )
        ...
        self.movs(["rcx", f"[{variable}]"], ["rdx", 1])
        ...
        self.movs(["[rcx]", "rax"], ["[rcx - 8]", "rbx"])

        self.jump(self.counters["indent"], "assign_end_mark")
        self.mark()

        self.compare(
            "rax", "!LIST_IDENTIFIER", "ne", self.counters["indent"], "assign_mark"
        )
        self.mov("rdx", 0)
        self.pushs("rax", "rbx")
        self.operations(["imul", "rbx", 2], ["add", "[!frames_counter]", "rbx"])
    ...
    def compile_ForNode(self, node: ForNode):
        ...
        self.compile(
            BinaryOperationNode(
                VariableAccessNode(node.variable_name, []), Token(LESS), node.end_node
            )
        )
        self.pops("rax", "rbx")
        ...
        self.compile(
            VariableAssignNode(
                ...
                [],
                BinaryOperationNode(
                    VariableAccessNode(node.variable_name, []), Token(ADDITION), step
                ),
            )
        )
        ...
        self.replace_mark(
            self.counters["indent"], "loop_iteration_mark", iteration_mark
        )
    ...
    def compile_FunctionDefinitionNode(self, node: FunctionDefinitionNode):
        function_name = (
            ...
            else f"!функция{self.counters["lambda"]}"
        )
        ...
        for index, argument_name in enumerate(node.argument_names):
            compiler.mov(
                "rax", f"[rbp + 8 * ({len(node.argument_names)} - {index} + 2)]"
            )
            compiler.push("rax")
        ...
        for index, variable in enumerate(local_variables):
            compiler.movs(
                ["rax", f"[rbp - 8 * ({index + 1} * 2)]"], [f"[{variable}]", "rax"]
            )
